...
class Enc(nn.Module):
    ...
    def __init__(self, latentDim_w, latentDim_u, dist):
        ...
        # self.ll_c1_u = nn.Sequential(*[nn.ReLU(), nn.Linear(128, latentDim_u)])
        # self.ll_c2_u = nn.Sequential(*[nn.ReLU(), nn.Linear(128, latentDim_u)])
    ...
    def forward(self, x):
        ...
        if self.dist == 'Normal':
            return torch.cat((mu_w, mu_u), dim=-1), \
                torch.cat((F.softplus(lv_w) + Constants.eta, F.softplus(lv_u) + Constants.eta), dim=-1)
        else:
            return torch.cat((mu_w, mu_u), dim=-1), \
